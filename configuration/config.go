...
func Load(path string) (*Configuration, bool, error) {
	...
	if len(cfg.Mail.Username) > 0 && len(cfg.Mail.Password) > 0 {
		smtpAuth = smtp.PlainAuth("", cfg.Mail.Username, cfg.Mail.Password, cfg.Mail.Hostname+":"+cfg.Mail.Port)
	}

	cfg.Mail.Client = smtpAuth
